...
assert(headerHtml.includes('class="mobile-header"') && headerHtml.includes("data-mobile-menu-toggle") && headerHtml.includes("data-mobile-nav"), "Header should include a dedicated mobile header and menu panel.");
assert(layoutJs.includes("initMobileHeader") && layoutJs.includes("[data-mobile-menu-toggle]") && layoutJs.includes("aria-expanded"), "layout.js should initialize the mobile header menu toggle accessibly.");
assert(/\.mobile-header\s*{[\s\S]*?display:\s*none/.test(css) && /\.mobile-nav-panel\s*{[\s\S]*?display:\s*none/.test(css), "Mobile header and panel should stay hidden on desktop.");
assert(/@media \(max-width:\s*1024px\)[\s\S]*?\.header-bar\s*{[\s\S]*?display:\s*none/.test(css) && /@media \(max-width:\s*1024px\)[\s\S]*?\.mobile-header\s*{[\s\S]*?display:\s*flex/.test(css), "Tablet and mobile widths should replace the desktop pill header with the mobile header.");
