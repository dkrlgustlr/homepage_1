...
assert(/@media \(max-width:\s*640px\)[\s\S]*?\.case-menu-arrow\s*{[\s\S]*?width:\s*28px[\s\S]*?min-height:\s*48px[\s\S]*?font-size:\s*18px/.test(css) && /@media \(max-width:\s*640px\)[\s\S]*?\.case-menu li\s*{[\s\S]*?height:\s*48px/.test(css) && /@media \(max-width:\s*640px\)[\s\S]*?\.case-tab\s*{[\s\S]*?font-size:\s*13px/.test(css), "Narrow mobile case carousel controls should use shorter buttons and smaller text.");
