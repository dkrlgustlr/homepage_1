...
assert(/@media \(max-width:\s*760px\)[\s\S]*?\.consult-reference-right\s*{[\s\S]*?padding:\s*0/.test(css) && /@media \(max-width:\s*760px\)[\s\S]*?\.consult-reference-right \.sub-consult-form\s*{[\s\S]*?padding:\s*44px var\(--side-pad\) 56px/.test(css), "Mobile consult form section should remove the outer white gutter and keep spacing inside the dark panel.");
assert(/@media \(max-width:\s*760px\)[\s\S]*?\.consult-reference-right\s*{[\s\S]*?width:\s*100vw[\s\S]*?min-height:\s*720px[\s\S]*?margin-left:\s*calc\(50% - 50vw\)[\s\S]*?margin-right:\s*calc\(50% - 50vw\)/.test(css) && /@media \(max-width:\s*760px\)[\s\S]*?\.consult-reference-right \.sub-consult-form\s*{[\s\S]*?width:\s*100%[\s\S]*?max-width:\s*none[\s\S]*?min-height:\s*100%/.test(css), "Mobile consult dark panel should fill the viewport width and hold the background through the form height.");
