...
assert(/@media \(max-width:\s*1024px\)[\s\S]*?\.header-bar\s*{[\s\S]*?display:\s*block[\s\S]*?overflow-x:\s*auto/.test(css) && /@media \(max-width:\s*1024px\)[\s\S]*?\.top-nav\s*{[\s\S]*?display:\s*flex/.test(css), "Tablet and mobile widths should keep the PC pill header design in a compact horizontal layout.");
