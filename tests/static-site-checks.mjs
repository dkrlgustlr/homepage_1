...
assert(/@media \(max-width:\s*1024px\)[\s\S]*?\.case-right\s*{[\s\S]*?display:\s*contents/.test(css) && /@media \(max-width:\s*1024px\)[\s\S]*?\.case-right \.box-top\s*{[\s\S]*?order:\s*1[\s\S]*?padding:\s*34px 26px 18px/.test(css) && /@media \(max-width:\s*1024px\)[\s\S]*?\.case-left\s*{[\s\S]*?order:\s*2[\s\S]*?background:\s*#fff[\s\S]*?padding:\s*0 0 24px/.test(css), "Mobile case tabs should appear below the case heading text with controlled spacing.");
...
assert(/@media \(max-width:\s*1024px\)[\s\S]*?\.case-list-window\s*{[\s\S]*?order:\s*4[\s\S]*?margin-top:\s*0[\s\S]*?padding:\s*20px 26px 38px/.test(css), "Mobile case rows should sit under the tabs without extra top margin.");
