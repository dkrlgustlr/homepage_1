...
assert(/@media \(max-width:\s*1024px\)[\s\S]*?\.case-menu li\s*{[\s\S]*?height:\s*64px[\s\S]*?display:\s*flex/.test(css) && /@media \(max-width:\s*1024px\)[\s\S]*?\.case-tab\s*{[\s\S]*?height:\s*100%[\s\S]*?min-height:\s*0/.test(css), "Mobile case carousel tabs should keep equal height even when labels wrap to two lines.");
...
assert(indexHtml.includes("scrollIntoView") && indexHtml.includes("inline: \"center\"") && indexHtml.includes("scrollCaseMenuByPage"), "Clicking a mobile case carousel item should center the active tab and arrows should page the carousel.");
