...
assert((knowledgeHtml.match(/data-knowledge-modal-open/g) || []).length === 4, "Knowledge cards should open the four active article details without separate article pages.");
...
assert(knowledgeHtml.includes("압류 및 추심명령") && knowledgeHtml.includes("폐업사실증명") && knowledgeHtml.includes("임대차보증금"), "Knowledge article content should include more detailed professional context.");
...
assert((knowledgeCardMarkup.match(/class="article-thumb"/g) || []).length === 4 && (knowledgeCardMarkup.match(/class="article-body"/g) || []).length === 4, "Knowledge cards should use one shared thumbnail class and body containers.");
...
assert((knowledgeCardMarkup.match(/--article-thumb-image:\s*url\('/g) || []).length === 4, "Each knowledge thumbnail should pass only its image source through the shared CSS variable.");
...
assert(knowledgeArticles.length === 4, "Knowledge structured data should describe all four active knowledge cards.");
...
assert(knowledgeArticles.some((article) => /급여압류 통장압류/.test(article.keywords)) && knowledgeArticles.some((article) => /채무 자료/.test(article.keywords)), "Knowledge Article keywords should target practical search and answer queries.");
