...
assert(/@media \(max-width:\s*1024px\)[\s\S]*?\.case-tab-mobile\s*{[\s\S]*?width:\s*100%[\s\S]*?box-sizing:\s*border-box/.test(css), "Mobile case tab buttons should fill their grid slots so active and inactive tabs have matching widths.");
assert(/@media \(max-width:\s*640px\)[\s\S]*?\.case-mobile-arrow\s*{[\s\S]*?width:\s*28px[\s\S]*?height:\s*48px[\s\S]*?font-size:\s*18px/.test(css) && /@media \(max-width:\s*640px\)[\s\S]*?\.case-mobile-menu-list li\s*{[\s\S]*?height:\s*48px/.test(css) && /@media \(max-width:\s*640px\)[\s\S]*?\.case-tab-mobile\s*{[\s\S]*?font-size:\s*12px/.test(css), "Narrow mobile case controls should keep readable text while using shorter buttons.");
...
assert(/@media \(max-width:\s*1024px\)[\s\S]*?--mobile-section-pad-y:\s*96px[\s\S]*?--mobile-section-split-gap:\s*40px/.test(css), "Tablet mobile layout should define shared vertical section rhythm tokens.");
assert(/@media \(max-width:\s*640px\)[\s\S]*?--mobile-section-pad-y:\s*72px[\s\S]*?--mobile-section-split-gap:\s*36px/.test(css), "Narrow mobile layout should use compact shared vertical section rhythm tokens.");
assert(/@media \(max-width:\s*1024px\)[\s\S]*?\.case-section,\s*\.location-section,\s*\.footer\s*{[\s\S]*?padding-top:\s*var\(--mobile-section-pad-y\)[\s\S]*?padding-bottom:\s*var\(--mobile-section-pad-y\)/.test(css), "Core mobile sections should share the same top and bottom spacing.");
assert(/@media \(max-width:\s*1024px\)[\s\S]*?\.case-section,\s*\.knowledge-row,\s*\.consult-section,\s*\.location-section,\s*\.footer\s*{[\s\S]*?min-height:\s*auto/.test(css), "Mobile sections should be content-led instead of using mismatched fixed minimum heights.");
assert(/@media \(max-width:\s*1024px\)[\s\S]*?\.knowledge-left\s*{[\s\S]*?padding-top:\s*var\(--mobile-section-pad-y\)[\s\S]*?padding-bottom:\s*var\(--mobile-section-split-gap\)/.test(css) && /@media \(max-width:\s*1024px\)[\s\S]*?\.knowledge-right\s*{[\s\S]*?padding-top:\s*var\(--mobile-section-split-gap\)[\s\S]*?padding-bottom:\s*var\(--mobile-section-pad-y\)/.test(css), "Split mobile knowledge section should use shared outer spacing and a smaller internal gap.");
assert(/@media \(max-width:\s*1024px\)[\s\S]*?\.consult-reference-section\s*{[\s\S]*?--mobile-consult-pad-top:\s*var\(--mobile-section-pad-y\)[\s\S]*?--mobile-consult-pad-bottom:\s*var\(--mobile-section-split-gap\)[\s\S]*?--mobile-consult-form-top:\s*var\(--mobile-section-split-gap\)[\s\S]*?--mobile-consult-form-bottom:\s*var\(--mobile-section-pad-y\)/.test(css), "Split mobile consultation section should use the same outer rhythm as other sections.");
assert(/@media \(max-width:\s*1024px\)[\s\S]*?\.top-nav\s*{[\s\S]*?top:\s*0[\s\S]*?right:\s*0[\s\S]*?width:\s*min\(320px,\s*100vw\)[\s\S]*?max-width:\s*100vw/.test(css), "Open mobile navigation panel should attach flush to the top-right viewport edge.");
assert(/@media \(max-width:\s*1024px\)[\s\S]*?\.header\.is-mobile-menu-open\s*\.header-bar\s*{[\s\S]*?position:\s*fixed[\s\S]*?top:\s*0[\s\S]*?right:\s*0[\s\S]*?width:\s*100vw[\s\S]*?height:\s*100dvh/.test(css), "Open mobile navigation should reset the header bar container to the viewport edge.");
assert(/@media \(max-width:\s*640px\)[\s\S]*?\.top-nav\s*{[\s\S]*?right:\s*0[\s\S]*?width:\s*min\(300px,\s*100vw\)/.test(css), "Narrow mobile navigation panel should not reserve a right-side gutter when opened.");
...
assert(/@media \(max-width:\s*1024px\)[\s\S]*?\.case-tab-mobile\s*{[\s\S]*?font-size:\s*12px/.test(finalTypographyBlock) && /@media \(max-width:\s*640px\)[\s\S]*?\.case-tab-mobile\s*{[\s\S]*?font-size:\s*12px/.test(finalTypographyBlock), "Mobile case button text should be reasserted after shared typography rules without shrinking below readable size.");
...
assert(/const sectionOverflowOffset\s*=\s*\(section\)\s*=>\s*\([\s\S]*?section\.matches\("\.consult-reference-section"\)[\s\S]*?Math\.max\(0,\s*section\.offsetHeight\s*-\s*usableViewportHeight\(\)\)[\s\S]*?\)/.test(layoutJs) && /const sectionTop\s*=\s*\(section\)\s*=>\s*Math\.min\(section\.offsetTop\s*\+\s*sectionOverflowOffset\(section\),\s*maxScrollTop\(\)\)/.test(layoutJs), "Desktop snap should bottom-align oversized consult sections above the fixed bottom bar.");
...
assert(/@media \(min-width:\s*1181px\) and \(max-height:\s*960px\)[\s\S]*?\.consult-reference-left\s*{[\s\S]*?--consult-left-content-shift:\s*-64px[\s\S]*?padding:\s*calc\(var\(--header-h\)\s*\+\s*24px\)[\s\S]*?38px[\s\S]*?\.consult-check-list li\s*{[\s\S]*?min-height:\s*56px/.test(css), "Short desktop consult sections should compact the left content so the 04 row stays above the fixed bottom bar.");
...
assert(!headerHtml.includes("class=\"hamburger\"") && !layoutJs.includes("class=\"hamburger\""), "Header should not render the old top-right hamburger menu.");
assert(headerHtml.includes("data-mobile-menu-toggle") && headerHtml.includes('id="mobile-nav-panel"') && headerHtml.includes("data-mobile-menu-close"), "Header should expose a mobile hamburger button and right-side drawer menu.");
assert(layoutJs.includes("initMobileNav") && layoutJs.includes("is-mobile-menu-open") && layoutJs.includes("aria-expanded"), "layout.js should initialize the mobile drawer menu and keep the toggle state accessible.");
...
assert(!headerHtml.includes("mobile-header") && !layoutJs.includes("initMobileHeader") && !css.includes(".mobile-header"), "Mobile should not use a separate newly designed header.");
assert(/@media \(max-width:\s*1024px\)[\s\S]*?\.header-bar\s*{[\s\S]*?left:\s*auto[\s\S]*?right:\s*18px[\s\S]*?width:\s*max-content[\s\S]*?max-width:\s*calc\(100vw - 36px\)[\s\S]*?overflow:\s*visible/.test(css) && /@media \(max-width:\s*1024px\)[\s\S]*?\.header-bar-inner\s*{[\s\S]*?justify-content:\s*flex-start[\s\S]*?width:\s*auto/.test(css) && /@media \(max-width:\s*1024px\)[\s\S]*?\.mobile-menu-toggle\s*{[\s\S]*?display:\s*inline-flex/.test(css), "Tablet and mobile headers should shrink to the consultation pill and hamburger without a blank middle gap.");
assert(/@media \(max-width:\s*1024px\)[\s\S]*?\.mobile-menu-toggle\s*{[\s\S]*?background:\s*#31353C[\s\S]*?color:\s*#fff/.test(css) && /@media \(max-width:\s*1024px\)[\s\S]*?\.header\.is-mobile-menu-open \.mobile-menu-toggle\s*{[\s\S]*?background:\s*#31353C[\s\S]*?color:\s*#fff/.test(css), "Mobile hamburger button should use the unified dark header background in both closed and open states.");
assert(/@media \(max-width:\s*1024px\)[\s\S]*?\.header\.is-mobile-menu-open \.mobile-menu-toggle\s*{[\s\S]*?position:\s*fixed[\s\S]*?top:\s*0[\s\S]*?right:\s*0[\s\S]*?width:\s*56px[\s\S]*?height:\s*56px[\s\S]*?border-radius:\s*0 0 0 16px/.test(css), "Open mobile drawer close button should sit flush against the top-right viewport corner.");
assert(/@media \(max-width:\s*1024px\)[\s\S]*?\.header\.is-mobile-menu-open \.mobile-menu-toggle span\s*{[\s\S]*?width:\s*24px[\s\S]*?height:\s*2px/.test(css), "Open mobile drawer close icon should use a cleaner wider X mark.");
assert(/@media \(max-width:\s*1024px\)[\s\S]*?\.top-nav\s*{[\s\S]*?position:\s*fixed[\s\S]*?right:\s*0[\s\S]*?transform:\s*translateX\(100%\)/.test(css) && /@media \(max-width:\s*1024px\)[\s\S]*?\.header\.is-mobile-menu-open \.top-nav\s*{[\s\S]*?transform:\s*translateX\(0\)/.test(css), "Mobile nav links should live in a right-side drawer that opens from the hamburger button.");
assert(/@media \(max-width:\s*1024px\)[\s\S]*?\.top-nav\s*{[\s\S]*?background:\s*linear-gradient\([\s\S]*?url\("mockup_assets\/beommusa_office_photo_v2\.png"\)[\s\S]*?border-left:\s*1px solid rgba\(255,255,255,0\.14\)[\s\S]*?isolation:\s*isolate/.test(css), "Mobile drawer should use a custom dark brand panel instead of a plain white menu.");
assert(/@media \(max-width:\s*1024px\)[\s\S]*?\.top-nav::before\s*{[\s\S]*?height:\s*5px[\s\S]*?background:\s*var\(--primary\)/.test(css), "Mobile drawer should include a primary-blue top accent.");
assert(/@media \(max-width:\s*1024px\)[\s\S]*?\.top-nav a::before\s*{[\s\S]*?counter-increment:\s*mobile-nav[\s\S]*?content:\s*"0" counter\(mobile-nav\)/.test(css) && /@media \(max-width:\s*1024px\)[\s\S]*?\.top-nav a::after\s*{[\s\S]*?transform:\s*rotate\(45deg\)/.test(css), "Mobile drawer links should use numbered rows with arrow affordances.");
assert(/@media \(max-width:\s*1024px\)[\s\S]*?\.fixed-side,\s*\.bottom-consult\s*{[\s\S]*?display:\s*none !important/.test(css), "Mobile layouts should hide both the floating side banner and the fixed bottom consultation banner.");
assert(/@media \(max-width:\s*768px\)[\s\S]*?:root\s*{[\s\S]*?--mobile-page-max:\s*768px[\s\S]*?--mobile-content-max:\s*430px/.test(css), "Mobile-only layout should be defined from a 768px breakpoint with shared width tokens.");
assert(/@media \(max-width:\s*768px\)[\s\S]*?\.hero,\s*\.case-section,\s*\.knowledge-row,\s*\.consult-section,\s*\.location-section,\s*\.footer\s*{[\s\S]*?max-width:\s*var\(--mobile-page-max\)[\s\S]*?margin-inline:\s*auto/.test(css), "Main mobile sections should share the 768px page width rule.");
assert(/@media \(max-width:\s*768px\)[\s\S]*?\.main-title-box,\s*\.lawyer-right,\s*\.case-inner,\s*\.knowledge-left > \*,\s*\.knowledge-right > \*,\s*\.consult-left > \*,\s*\.consult-right,\s*\.location-inner,\s*\.footer > \*\s*{[\s\S]*?max-width:\s*var\(--mobile-content-max\)[\s\S]*?margin-inline:\s*auto/.test(css), "Mobile content blocks should use a shared centered content width instead of ad-hoc full-width layouts.");
assert(/@media \(max-width:\s*768px\)[\s\S]*?\.column-grid,\s*\.sub-grid,\s*\.case-study-grid,\s*\.service-area-grid,\s*\.faq-list,\s*\.article-list,\s*\.service-area-list\s*{[\s\S]*?grid-template-columns:\s*1fr/.test(css), "Mobile card grids should switch to one column at the 768px breakpoint.");
assert(/@media \(max-width:\s*768px\)[\s\S]*?\.form-row,\s*\.sub-form-row\s*{[\s\S]*?grid-template-columns:\s*1fr/.test(css), "Mobile forms should use one-column rows from the 768px breakpoint.");
assert(/@media \(max-width:\s*1024px\)[\s\S]*?\.hero\s*{[\s\S]*?height:\s*auto[\s\S]*?min-height:\s*760px/.test(css), "Mobile hero should use a section-specific height instead of viewport-fitting height.");
assert(/@media \(max-width:\s*1024px\)[\s\S]*?\.case-section\s*{[\s\S]*?min-height:\s*1080px/.test(css) && /@media \(max-width:\s*1024px\)[\s\S]*?\.consult-section\s*{[\s\S]*?min-height:\s*940px/.test(css) && /@media \(max-width:\s*1024px\)[\s\S]*?\.location-section\s*{[\s\S]*?min-height:\s*760px/.test(css), "Mobile main sections should use explicit section height baselines.");
assert(/@media \(max-width:\s*768px\)[\s\S]*?\.consult-section\.consult-reference-section\s*{[\s\S]*?min-height:\s*auto/.test(css), "Mobile consult reference section should not keep viewport-based min-height that creates top whitespace.");
assert(/@media \(max-width:\s*768px\)[\s\S]*?\.consult-reference-section\s*{[\s\S]*?--mobile-consult-pad-top:\s*76px[\s\S]*?--mobile-consult-pad-bottom:\s*42px[\s\S]*?--mobile-consult-gap:\s*24px/.test(css), "Mobile consult section should define one spacing rule for top, bottom, and block gaps.");
assert(/@media \(max-width:\s*768px\)[\s\S]*?\.consult-reference-left\s*{[\s\S]*?justify-content:\s*flex-start[\s\S]*?padding:\s*var\(--mobile-consult-pad-top\) var\(--side-pad\) var\(--mobile-consult-pad-bottom\)/.test(css), "Mobile consult left panel should use the shared spacing rule and align content from the top.");
assert(/@media \(max-width:\s*768px\)[\s\S]*?\.consult-reference-right\s*{[\s\S]*?padding:\s*0/.test(css) && /@media \(max-width:\s*768px\)[\s\S]*?\.consult-reference-right \.consult-form,\s*\.consult-reference-right \.sub-consult-form\s*{[\s\S]*?padding:\s*44px 0 56px/.test(css), "Mobile consult section should remove only the outer white gutter and keep form spacing inside the inset widget.");
assert(/@media \(max-width:\s*768px\)[\s\S]*?\.consult-reference-right\s*{[\s\S]*?width:\s*100vw[\s\S]*?min-height:\s*720px[\s\S]*?margin-left:\s*calc\(50% - 50vw\)[\s\S]*?margin-right:\s*calc\(50% - 50vw\)/.test(css) && /@media \(max-width:\s*768px\)[\s\S]*?\.consult-reference-right \.consult-form,\s*\.consult-reference-right \.sub-consult-form\s*{[\s\S]*?width:\s*min\(520px,\s*calc\(100% - \(var\(--side-pad\) \* 2\)\)\)[\s\S]*?max-width:\s*520px[\s\S]*?margin:\s*0 auto[\s\S]*?min-height:\s*auto/.test(css), "Mobile consult background should fill the viewport while the form widget keeps the approved side margins.");
