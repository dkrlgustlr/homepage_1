...
assert(/@media \(min-width:\s*1181px\) and \(max-height:\s*820px\)[\s\S]*?\.consult-reference-left\s*{[\s\S]*?--consult-left-content-shift:\s*-72px[\s\S]*?padding:\s*calc\(var\(--header-h\)\s*\+\s*14px\)[\s\S]*?28px[\s\S]*?\.consult-check-list li\s*{[\s\S]*?min-height:\s*52px/.test(css), "Short desktop consult sections should compact the left content so the 04 row stays above the fixed bottom bar.");
