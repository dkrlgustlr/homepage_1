...
assert((knowledgeHtml.match(/data-knowledge-modal-open/g) || []).length === 1, "Knowledge cards should open the single active article detail without separate article pages.");
...
assert(knowledgeHtml.includes("압류 및 추심명령") && knowledgeHtml.includes("금지명령") && knowledgeHtml.includes("중지명령"), "Knowledge article content should include more detailed professional context.");
...
assert((knowledgeCardMarkup.match(/class="article-thumb"/g) || []).length === 1 && (knowledgeCardMarkup.match(/class="article-body"/g) || []).length === 1, "Knowledge cards should use one shared thumbnail class and body containers.");
...
assert((knowledgeCardMarkup.match(/--article-thumb-image:\s*url\('/g) || []).length === 1, "Each knowledge thumbnail should pass only its image source through the shared CSS variable.");
...
assert(knowledgeArticles.length === 1, "Knowledge structured data should describe the single active knowledge card.");
...
assert(knowledgeArticles.some((article) => /급여압류 통장압류/.test(article.keywords)), "Knowledge Article keywords should target practical search and answer queries.");
