...
assert(/@media \(max-width:\s*1024px\)[\s\S]*?\.case-menu-shell\s*{[\s\S]*?grid-template-columns:\s*34px minmax\(0,\s*1fr\) 34px[\s\S]*?width:\s*calc\(100% - 52px\)[\s\S]*?max-width:\s*420px[\s\S]*?margin:\s*0 auto/.test(css), "Mobile case carousel should use a centered max-width shell with clear side breathing room.");
assert(/@media \(max-width:\s*1024px\)[\s\S]*?\.case-menu-arrow\s*{[\s\S]*?display:\s*inline-flex/.test(css), "Mobile case carousel arrows should be visible.");
...
assert(/@media \(max-width:\s*1024px\)[\s\S]*?\.case-right\s*{[\s\S]*?display:\s*contents/.test(css) && /@media \(max-width:\s*1024px\)[\s\S]*?\.case-right \.box-top\s*{[\s\S]*?order:\s*1[\s\S]*?padding:\s*34px 26px 18px/.test(css) && /@media \(max-width:\s*1024px\)[\s\S]*?\.case-left\s*{[\s\S]*?order:\s*5[\s\S]*?background:\s*#fff[\s\S]*?padding:\s*0 0 24px/.test(css), "Mobile case tabs should sit at the bottom of the case card with controlled spacing.");
...
assert(/@media \(max-width:\s*1024px\)[\s\S]*?\.case-list-window\s*{[\s\S]*?order:\s*4[\s\S]*?margin-top:\s*0[\s\S]*?padding:\s*20px 26px 24px/.test(css), "Mobile case rows should sit above the bottom tabs without extra top margin.");
...
assert(/@media \(max-width:\s*760px\)[\s\S]*?\.consult-section\.consult-reference-section\s*{[\s\S]*?min-height:\s*auto/.test(css), "Mobile consult reference section should not keep viewport-based min-height that creates top whitespace.");
assert(/@media \(max-width:\s*760px\)[\s\S]*?\.consult-reference-section\s*{[\s\S]*?--mobile-consult-pad-top:\s*76px[\s\S]*?--mobile-consult-pad-bottom:\s*42px[\s\S]*?--mobile-consult-gap:\s*24px/.test(css), "Mobile consult section should define one spacing rule for top, bottom, and block gaps.");
assert(/@media \(max-width:\s*760px\)[\s\S]*?\.consult-reference-left\s*{[\s\S]*?justify-content:\s*flex-start[\s\S]*?padding:\s*var\(--mobile-consult-pad-top\) var\(--side-pad\) var\(--mobile-consult-pad-bottom\)/.test(css), "Mobile consult left panel should use the shared spacing rule and align content from the top.");
assert(/@media \(max-width:\s*760px\)[\s\S]*?\.consult-reference-right\s*{[\s\S]*?padding:\s*0/.test(css) && /@media \(max-width:\s*760px\)[\s\S]*?\.consult-reference-right \.consult-form,\s*\.consult-reference-right \.sub-consult-form\s*{[\s\S]*?padding:\s*44px 0 56px/.test(css), "Mobile consult section should remove only the outer white gutter and keep form spacing inside the inset widget.");
assert(/@media \(max-width:\s*760px\)[\s\S]*?\.consult-reference-right\s*{[\s\S]*?width:\s*100vw[\s\S]*?min-height:\s*720px[\s\S]*?margin-left:\s*calc\(50% - 50vw\)[\s\S]*?margin-right:\s*calc\(50% - 50vw\)/.test(css) && /@media \(max-width:\s*760px\)[\s\S]*?\.consult-reference-right \.consult-form,\s*\.consult-reference-right \.sub-consult-form\s*{[\s\S]*?width:\s*min\(520px,\s*calc\(100% - \(var\(--side-pad\) \* 2\)\)\)[\s\S]*?max-width:\s*520px[\s\S]*?margin:\s*0 auto[\s\S]*?min-height:\s*auto/.test(css), "Mobile consult background should fill the viewport while the form widget keeps the approved side margins.");
